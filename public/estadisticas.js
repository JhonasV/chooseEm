  ...
  if (estadisticaContainer) {
    ...
    if (partido._id === estadistica.partidoId && mayor[0]._id === partido._id) {
      let elementoHtml = `
        ...
        estadistica.porcentaje
      }" aria-valuemin="0" aria-valuemax="100">${estadistica.porcentaje}%</div>
                                      </div>
                        </div>
                    </div>
                </div>
      ...
      partido._id === estadistica.partidoId &&
      mayor[0]._id !== partido._id
    ) {
      let elementoHtml = `
        <div>
        <div class="partido-porcentaje">
...
const catalogoCandidatos = partido => {
  ...
  if (catalogo) {
    let elementoHTML = `
        ...
        <div class="col-md-3">
            <input type="radio" id="candidato-select-${partido._id}" value="${
      partido._id
    }" name="candidato">
